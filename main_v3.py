...
def teaching(model_stu, device):
    ...
    static_tch = build_teacher(args, model_stu, device)

    #build static
    if args.mode == "teaching_standard_distill" or args.mode == 'teaching_distill_mask':
        # initialize four models
        assert args.enable_dino 
    ...
    if args.distributed:
        ...
        static_tch = DistributedDataParallel(static_tch, device_ids=[args.gpu])  if static_tch is not None else None     
    ...
    for epoch in range(args.epoch):
        ...
        phase = 10
        for piece_id in range(piece_num):
            if args.enable_dino:
                count = piece_id + epoch * piece_num
                if count < phase:
                    model_tch.module.dino_factor.data =  args.dino_weight * torch.tensor( ((count+1)/phase) ** args.dino_alpha).cuda()
                    # model_tch.module.dino_factor.data = torch.tensor(math.sin(count/10 * math.pi/2) * 0.5).cuda()
            ...
            if is_main_process() and args.enable_dino:
                print(f"Piece ID: {piece_id}/{piece_num}")
            ...
            if args.mode == "teaching_mask":
                loss_train, loss_target_dict, target_fetcher = train_one_epoch_teaching_mask_with_piece(
            ...
            elif args.mode == "teaching_distill_mask":
                loss_train, loss_target_dict, target_fetcher = train_one_epoch_teaching_mask_with_piece_distill(
                    student_model=model_stu,
                    static_teacher_model = static_tch,
                    teacher_model=model_tch,
                    init_student_model=init_model_stu,
                    criterion_pseudo=criterion_pseudo,
                    criterion_pseudo_weak=criterion_pseudo_weak,
                ...
                loss_train, loss_target_dict, target_fetcher = train_one_epoch_teaching_standard_with_piece(
                    student_model=model_stu,
                    teacher_model=model_tch,
                    criterion_pseudo=criterion_pseudo,
                    target_loader=target_loader,
            ...
            if is_main_process() :
                ...
                print('eval teacher')
                write_ap50(epoch*piece_num+piece_id, 'teaching_teacher', map50_tch, ap50_per_class_teacher, idx_to_class)
                print('eval stdent')
                write_ap50(epoch*piece_num+piece_id, 'teaching_student', map50_stu, ap50_per_class_student, idx_to_class)
            ...
            if (piece_id+1) % 5 == 0:
                save_ckpt(model_tch, output_dir/f'tch_epoch{epoch:02}_pieceid{piece_id}.pth', args.distributed)
                save_ckpt(model_stu, output_dir/f'stu_epoch{epoch:02}_pieceid{piece_id}.pth', args.distributed)
    end_time = time.time()
    total_time_str = str(datetime.timedelta(seconds=int(end_time - start_time)))
...
def main():
    ...
    elif args.mode == "teaching_standard" or args.mode == "teaching_mask" or args.mode == 'teaching_distill_mask':
